...
Original file is located at
    https://colab.research.google.com/drive/1bYaOKWV-Z95xZU7UpJf5HsFZYrjoWLz-
"""
...
# ====== 1. 儲存原本的硬標籤提交檔 (交給 Kaggle 用的) ======
sub = pd.DataFrame({
    'id': test_ids,
    target_col: final_test_preds
})
sub[target_col] = sub[target_col].map(idx2target)
sub.to_csv("lgb_with_TE_hard.csv", index=False)


# ====== 2. 【新增】儲存軟標籤機率檔 (我們未來融合用的王牌) ======
# 利用 idx2target 字典，動態生成對應的機率欄位名稱 (例如 prob_Low, prob_Medium, prob_High)
prob_cols = [f"prob_{idx2target[i]}" for i in range(num_classes)]

# 把機率矩陣轉換成 DataFrame
prob_df = pd.DataFrame(final_test_probs, columns=prob_cols)

# 把 id 加進去，並確保 id 放在第一欄
prob_df['id'] = test_ids.values
prob_df = prob_df[['id'] + prob_cols]

# 儲存機率檔
prob_df.to_csv("my_0.98030_probabilities.csv", index=False)

print("\n🎉 輸出完成！")
print("1. 已產生硬標籤檔: lgb_with_TE_hard.csv")
print("2. 已產生機率檔: my_0.98030_probabilities.csv 💎")